...
{
	/* Enable GPIO clock for FMU_CH1. */
	rcc_periph_clock_enable(RCC_GPIOE);

	/* Enable led as output */
	gpio_mode_setup(FMU_CH1_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, FMU_CH1_PIN);
	gpio_set(FMU_CH1_PORT, FMU_CH1_PIN);
}
...
void tim2_isr(void)
{
	if (timer_get_flag(TIM2, TIM_SR_CC1IF)) {
		...
		gpio_toggle(FMU_CH1_PORT, FMU_CH1_PIN);
	}
}
#endif 
